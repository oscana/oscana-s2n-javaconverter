 ...
 *
 */
public class S2AbstractServiceFindAllHandler extends AbstractProgramConvertHandler {

    private Node classNode;
    private String genericsType;

    public S2AbstractServiceFindAllHandler(Node classNode, ClassNameResolver tcnr) {
        super(tcnr);
    ...
    protected boolean handle(Token token, BracketLevel bracketLevel, Node lineNode) {
        ...
        switch (step) {
        case 0:
            if ((token.getText().equals("findAll"))) {
                step++;
                addCommand(new ChangeToTxtCommand(token, CN_UNIVERSALDAO + ".findAll"));
