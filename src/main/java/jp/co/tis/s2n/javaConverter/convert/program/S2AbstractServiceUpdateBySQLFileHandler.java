 ...
 * updateBySqlFile(path, parameter).execute() → DbConnectionContext.getConnection().prepareParameterizedSqlStatementBySqlId(sqlId).executeUpdateByObject(params);
 *
 * @author Fumihiko Yamamoto
 *
 */
public class S2AbstractServiceUpdateBySQLFileHandler extends AbstractProgramConvertHandler {

    private ParamUtil inParams = new ParamUtil();

    public S2AbstractServiceUpdateBySQLFileHandler(ClassNameResolver tcnr) {
        super(tcnr);
    }
    ...
    protected boolean handle(Token token, BracketLevel bracketLevel, Node lineNode) {
        switch (step) {
        case 0:
            if ((token.getText().equals("updateBySqlFile"))) {
                step++;
