package jp.co.tis.sample.action.test;
...
import jp.co.tis.oscana.exception.SystemErrorException;
import jp.co.tis.oscana.seasar.struts.annotation.Forward;
 ...
 */
@RequestScoped
public class ConvertInsertRouteXmlAction extends AbstractSearchAction  implements Serializable{
    ...
    public HttpResponse index(HttpRequest nabRequest, ExecutionContext nabContext) {
        ...
        return OscanaHttpResourceConverUtil.createHttpResponse( "SampleSearchList.jsp", this, nabRequest, nabContext, "ConvertInsertRouteXml");
    }
    ...
    @OnError(type = ApplicationException.class, path = "/WEB-INF/view/test/convertInsertRouteXml/SampleSearchList.jsp")
    public HttpResponse doSearch(HttpRequest nabRequest, ExecutionContext nabContext) {
        ...
        if (sampleSearchListForm.isEnvTypeHonban()) {
            ...
            for (Tbsample tbsample : tbsampleList) {
                ...
                Tbsample sample = new CreateAndCopy<Tbsample>(Tbsample.class, tbsample).execute();

                if (StringUtils.isNotBlank(sample.testdata)) {
                    sample.testdata = convertTest(StringUtil.trimRight(tbsample.testdata));
                }

                if (StringUtils.isNotBlank(sample.initialvector)) {
                    sample.initialvector = convertTest(StringUtil.trimRight(tbsample.initialvector));
                }
            ...
            for (Tbsample bufsample : thesampleList) {
                ...
                Tbsample sample = new CreateAndCopy<Tbsample>(Tbsample.class, bufsample).execute();

                if (StringUtils.isNotBlank(bufsample.testdata)) {
                    sample.testdata = convertTest(StringUtil.trimRight(bufsample.testdata));
                }

                if (StringUtils.isNotBlank(bufsample.initialvector)) {
                    sample.initialvector = convertTest(StringUtil.trimRight(bufsample.initialvector));
                }
        ...
        return OscanaHttpResourceConverUtil.createHttpResponse( sampleSearchList(), this, nabRequest, nabContext, "ConvertInsertRouteXml");
    }
    ...
    public HttpResponse fwCreate(HttpRequest nabRequest, ExecutionContext nabContext) {
        ...
        if (sampleSearchListForm.isEnvTypeHonban()) {
            ...
            if (recordCnt == 0) {
                ...
                return OscanaHttpResourceConverUtil.createHttpResponse( doSearch(nabRequest, nabContext), this, nabRequest, nabContext, "ConvertInsertRouteXml");
            }
        ...
        // レコードが存在した場合はForward
        return OscanaHttpResourceConverUtil.createHttpResponse( "/sample/sampleInsert/", this, nabRequest, nabContext, "ConvertInsertRouteXml");
    }
    ...
    private BeanMap getWhere(SampleSearchListForm sampleSearchListForm) {
        ...
        BeanMap map = new CreateAndCopy<BeanMap>(BeanMap.class, sampleSearchListForm).prefix("search_").execute();
    ...
    private String convertTest(String befUnScr) {
